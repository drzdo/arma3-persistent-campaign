/* ROS_OPFOR_AI_Revive system v4.0 - by RickOShay
...
INSTALLATION:
1. Copy the ROS_OPFOR_AI_revive folder to your mission root.
...
[] execvm "ROS_OPFOR_AI_revive\scripts\ROS_OPFOR_monitorhealth.sqf";
...
// ie: ROS_OPFOR_captiveBL = [hostage1, spy1, sniper1];
ROS_OPFOR_captiveBL = [];
...
// Debug testing
ROS_OPFOR_AIRevive_debug = false;

publicVariable "ROS_OPFOR_AIreviveScope";
ROS_OPFOR_incapacitated = [];
ROS_OPFOR_Processing = [];
publicVariable "ROS_OPFOR_Processing";
ROS_OPFOR_bloodPools = [];
ROS_OPFOR_medics = [];
ROS_OPFOR_medicDist = 1000;
...
// Medical Items
ROS_OPFOR_medicItems = ["Medikit","FirstAidKit","fak"];
...
if (isServer) then {
	...
	ROS_OPFOR_ALLUnitsArray = [];
	publicVariable "ROS_OPFOR_ALLUnitsArray";

	ROS_OPFOR_Fnc_unitArrayEHupdate = {

		// Get All Units array from server

		// player group include players (default) 	* LOCAL TO EACH LEADER GROUP PLAYER
		if (ROS_OPFOR_AIreviveScope == 1) then {ROS_OPFOR_AllUnitsArray = allunits select {isplayer leader _x && simulationEnabled _x}};

		// player side include players 				* LOCAL TO LEADER GROUP PLAYER AND SERVER
		if (ROS_OPFOR_AIreviveScope == 2) then {ROS_OPFOR_AllUnitsArray = allunits select {(side _x == ROS_OPFOR_RevSide or side _x == civilian) && simulationEnabled _x}};

		// player side exclude players 				* LOCAL TO SERVER AND PLAYER (if player has group AI units)
		if (ROS_OPFOR_AIreviveScope == 3) then {ROS_OPFOR_AllUnitsArray = allunits select {(side _x == ROS_OPFOR_RevSide or side _x == civilian) && !(isPlayer _x) && simulationEnabled _x}};

		publicVariable "ROS_OPFOR_ALLUnitsArray";

		waitUntil {count ROS_OPFOR_ALLUnitsArray >0};
		...
		// Add HDEH and addaction and Grp vars
		{
			...
			if !(_x getVariable ["HDEHadded",false]) then {
							...
							if !(lifestate _unit == "INCAPACITATED") then {
								[_unit] spawn {
									...
									if (random 1 >0.98) then {
										...
										ROS_OPFOR_incapacitated pushBackUnique _unit;
										publicVariable "ROS_OPFOR_incapacitated";
										if (count units group _unit ==1) then {
											if (_unit getVariable ["ROS_OPFOR_OrigPos",[]] isEqualTo []) then {
												_pos = (getPosATL _unit);
												_dir = (getdir _unit);
												_unit setVariable ["ROS_OPFOR_OrigPos", _pos, true];
												_unit setVariable ["ROS_OPFOR_OrigDir", _dir, true];
											};
										};
									};
								};
				...
				if (isPlayer _x && !(_x getVariable ["ROS_OPFOR_RevActionAdded",false])) then {
					// Zdo addition: no need in REVIVE action, this is for AI only.
					//[_x, ["<t color='#FF8000'>REVIVE</t>",{params ["_target", "_caller", "_actionId"]; [_caller] execVM "ROS_OPFOR_AI_revive\scripts\ROS_OPFOR_PlayerReviveAI.sqf";},[],8,true,true,"","!isplayer cursorObject && _this distance cursorObject < 2.2 && lifeState cursorObject == 'INCAPACITATED' && animationstate _this find 'medic' ==-1"]] remoteexec ["addAction", _x];

					_x setVariable ["ROS_OPFOR_RevActionAdded",true,true];
				}; // player
		...
		} foreach ROS_OPFOR_ALLUnitsArray;

		ROS_OPFOR_AllUnitsArray

	}; // end ROS_OPFOR_Fnc_unitArrayEHupdate

	// Update ALLUNITS array
	[] call ROS_OPFOR_Fnc_unitArrayEHupdate; // returns ROS_OPFOR_AllUnitsArray ~0.05s
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
// Medic comments
ROS_OPFOR_Fnc_medicLines = {
	params ["_incap", "_medic"];
	if (ROS_OPFOR_allowComments && isplayer _incap) then {
...
ROS_OPFOR_Fnc_medicThanks = {
	params ["_incap", "_medic"];
	if (ROS_OPFOR_allowComments) then {
...
ROS_OPFOR_Fnc_groan = {
	...
	if (isnil "ROS_OPFOR_lastMoanIndx") then {
		ROS_OPFOR_lastMoanIndx = 0;
	} else {
		if (ROS_OPFOR_lastMoanIndx +1 >3) then {
			ROS_OPFOR_lastMoanIndx = 0;
		} else {
			ROS_OPFOR_lastMoanIndx = ROS_OPFOR_lastMoanIndx +1;
		};
	};

	_sndPathM = ["A3\sounds_f\characters\human-sfx\Person0\","A3\sounds_f\characters\human-sfx\Person1\","A3\sounds_f\characters\human-sfx\Person2\","A3\sounds_f\characters\human-sfx\Person3\"] select ROS_OPFOR_lastMoanIndx;
	_moanSndArray = [moanP0, moanP1, moanP2, moanP3] select ROS_OPFOR_lastMoanIndx;
...
ROS_OPFOR_Fnc_blood = {
	...
	if (count _nearestBlood ==0) then {
		...
		ROS_OPFOR_bloodPools pushBackUnique _blood;
...
ROS_OPFOR_Fnc_RevSmoke = {
	...
	_ttl = 0;
	_smokeSpawnDist = 10;
	_nearEnemy = [_medic] call ROS_OPFOR_Fnc_NearEnemy;
	...
	if (getPosATL _incap select 2 <1) then {
		...
		if (ROS_OPFOR_RevSmokeCol == "random") then {
			_col = selectRandom _colors;
		} else {
			_col = ROS_OPFOR_RevSmokeCol;
		};
...
ROS_OPFOR_Fnc_NearEnemy = {
	params ["_unit"];

	_nearEnemies = [];
	_nearEnemy = objNull;

	_enemySides = (ROS_OPFOR_RevSide call BIS_fnc_enemySides);
...
ROS_OPFOR_Fnc_rank_Capitalization = {
	params ["_unitRank"];
	_new1stChar = "";
	_lowerTxt = "";
...
ROS_OPFOR_Fnc_resetAI = {
	...
	if (_unit in ROS_OPFOR_Processing) then {_unit dofollow leader (group _unit)};
	ROS_OPFOR_Processing = ROS_OPFOR_Processing - [_unit];
	publicVariable "ROS_OPFOR_Processing";
	_unit setvariable ["reviveTimeLimit",0,true];
};

ROS_OPFOR_Fnc_AI_healSelf = {
	params ["_unit"];
	...
	if (alive _unit) then {
		...
		_nearEnemy = [_unit] call ROS_OPFOR_Fnc_NearEnemy;
...
ROS_OPFOR_Fnc_UpdateIncapAndMedics = {

	// Add players to incap array
	{if (isPlayer _x && lifeState _x == "INCAPACITATED" && !(_x in ROS_OPFOR_incapacitated)) then {ROS_OPFOR_incapacitated = ROS_OPFOR_incapacitated + [_x]}} foreach ROS_OPFOR_AllUnitsArray;

	// Set and Clear Incap state
	if (count (ROS_OPFOR_AllUnitsArray select {alive _x}) >0) then {
		{

			if !(lifeState _x == "INCAPACITATED") then {ROS_OPFOR_incapacitated = ROS_OPFOR_incapacitated - [_x];};

			if ((_x getVariable ["incapTimeLimit",0])>0) then {
				// Time up
				if (time > (_x getVariable "incapTimeLimit") && lifeState _x == "INCAPACITATED") then {
					// Unit dies
					if !(ROS_OPFOR_autoRecover) then {
						_x setDamage 1;
						_rankI = [(rank _x)] call ROS_OPFOR_Fnc_rank_Capitalization;
						if (ROS_OPFOR_statusmsg or ROS_OPFOR_AIRevive_debug) then {[format ["%1 %2\nunfortunately\ndied from his injuries", _rankI, name _x]] remoteExec ["hintSilent",allPlayers]};
					} else {
						// Unit auto recovers
						_x setDamage 0;
					...
					ROS_OPFOR_Processing = ROS_OPFOR_Processing - [_x];
					publicVariable "ROS_OPFOR_Processing";
					// Remove nearest yellow markers if debug
					if (ROS_OPFOR_AIRevive_debug) then {_x call ROS_OPFOR_Fnc_delYmarkers};
				};
			} else {
				// Stamp incap with death/autorevive time
				if (_x getVariable ["incapTimeLimit",0] ==0) then {_x setVariable ["incapTimeLimit", (time + ROS_OPFOR_maxIncapTime), true]};
			};

		} foreach ROS_OPFOR_incapacitated;
	};

	// Remove captive and allowdamage states
	{
		if (captive _x && !(_x in ROS_OPFOR_captiveBL) && !(lifeState _x == "INCAPACITATED") && !(_x in ROS_OPFOR_Processing)) then {[_x,false] remoteExec ["setCaptive",_x]};
		//if (!(isDamageAllowed _x) && !(_x in ROS_OPFOR_Processing) && !(lifeState _x == "INCAPACITATED")) then {[_x,true] remoteExec ["allowDamage",_x]};
		if (time > (_x getvariable "reviveTimeLimit")) then {
			_x call ROS_OPFOR_Fnc_resetAI;
		};
	} foreach ROS_OPFOR_AllUnitsArray;
};
...
ROS_OPFOR_Fnc_Recycle_Fnc = {
	params ["_medic", "_incap"];

	if (ROS_OPFOR_AIRevive_debug) then {[format ["Incap: %1\nMedic: %2", _incap, _medic]] remoteExec ["hintsilent", allPlayers]};
	...
	while {alive _incap && alive _medic && lifestate _incap == "INCAPACITATED"} do {
		...
		if (_reviveTime > 0 && time > _reviveTime) then {
			...
			_medic call ROS_OPFOR_Fnc_resetAI;
			...
			ROS_OPFOR_Processing = ROS_OPFOR_Processing - [_incap];
			publicVariable "ROS_OPFOR_Processing";
			if (ROS_OPFOR_AIRevive_debug) then {
				["Medic reset\nTaking too long"] remoteExec ["hintsilent", allPlayers];
				_incap call ROS_OPFOR_Fnc_delYmarkers;
			};
		...
		// Incap auto recovers
		if (ROS_OPFOR_autoRecover && _incapTL > 0 && time > _incapTL) then {
			ROS_OPFOR_Processing = ROS_OPFOR_Processing - [_incap, _medic];
			publicVariable "ROS_OPFOR_Processing";
			_incap setVariable ["ROS_OPFOR_IncapMarker","",true];
			_incap setVariable ["reviveTimeLimit", 0, true];
...
ROS_OPFOR_Fnc_AIReviveUnits = {
	...
	if (isNil "_medic" or isnil "_incap") exitWith {
		if (alive _incap) then {
			if (ROS_OPFOR_AIRevive_debug) then {["Incap exists and Medic isnil"] remoteExec ["hintsilent", allPlayers]};
			ROS_OPFOR_Processing = ROS_OPFOR_Processing - [_incap];
			publicVariable "ROS_OPFOR_Processing";
		};
		...
		if (ROS_OPFOR_AIRevive_debug) then {["Medic reset > EXIT"] remoteExec ["hintsilent", allPlayers]};
	...
	// Check Medic can run
	_medic call ROS_OPFOR_Fnc_AI_healSelf;
	...
	if (alive _incap && alive _medic) then {
		if (ROS_OPFOR_moaning && isPlayer _medic) then {[_incap, _medic] spawn ROS_OPFOR_Fnc_groanWords};
		[_medic, _incap] remoteExecCall ["disableCollisionWith", 0, _medic];
		sleep 1;
		[[_medic, _incap], ROS_OPFOR_Fnc_blood] remoteExec ["spawn", 0, true];
	};
	...
	if (ROS_OPFOR_AIRevive_debug) then {_ymarker1 = createVehicle ["Sign_Arrow_Yellow_F", _revPos,[],0,"can_collide"];};
	...
	if (alive _incap && alive _medic && lifestate _incap == "INCAPACITATED") then {
		...
		// Add smoke
		if (ROS_OPFOR_RevSmokeOn) then {[_incap, _medic, _multiplier] spawn ROS_OPFOR_Fnc_RevSmoke};

		// Groaning sounds
		if (ROS_OPFOR_moaning) then {[_incap, _medic] spawn ROS_OPFOR_Fnc_groan};

		// Medic speaks
		[_incap, _medic] spawn ROS_OPFOR_Fnc_medicLines;

		// Add some medical waste
		[_medic] spawn ROS_OPFOR_Fnc_medwaste;
		_medic dowatch _incap;
		if (_medic distance _revpos >0.15) then {_medic setPosATL _revPos};

		if (isPlayer (leader _incap)) then {
			_rankI = [(rank _incap)] call ROS_OPFOR_Fnc_rank_Capitalization;
			_rankM = [(rank _medic)] call ROS_OPFOR_Fnc_rank_Capitalization;
		...
		if (ROS_OPFOR_allowComments && random 1 <0.5) then {[_incap, _medic] spawn ROS_OPFOR_Fnc_medicThanks};
	...
	ROS_OPFOR_Processing = ROS_OPFOR_Processing - [_medic, _incap];
	publicVariable "ROS_OPFOR_Processing";
	...
	//[_incap,true] remoteExec ["allowDamage",_incap];
	[_incap,false] remoteExec ["setCaptive",_incap];
	if (ROS_OPFOR_AIRevive_debug) then {["Medic reset\nRevive completed"] remoteExec ["hintsilent", allPlayers]};

	// Reset dowatch
	[_medic, objNull] remoteExec ["doWatch",_medic];

	// Delete Incap marker
	if !(_incap getVariable ["ROS_OPFOR_IncapMarker",""] == "") then {
		deleteMarker (_incap getVariable "ROS_OPFOR_IncapMarker");
		_incap setVariable ["ROS_OPFOR_IncapMarker","",true];
	};
	...
	if (ROS_OPFOR_AIRevive_debug) then {
		_incap call ROS_OPFOR_Fnc_delYmarkers;
		if (lifestate _incap != "incapacitated") then {
			ROS_OPFOR_TimeTocomplete = (time - _reviveTime);
			[format ["Revive Total time: %1", ROS_OPFOR_TimeTocomplete]] remoteExec ["hintSilent",allPlayers];
		} else {
	...
	*/
	...
	[_medic, _incap, _nearEnemy] spawn {
		...
		if (behaviour _medic != "COMBAT" && isNull _nearEnemy) then {
			if !((_medic getVariable ["ROS_OPFOR_medicOrigPos",[]]) isEqualTo []) then {
				_posm = (_medic getVariable ["ROS_OPFOR_medicOrigPos",[]]);
				_dirm = (_medic getVariable ["ROS_OPFOR_medicOrigDir",0]);
				_medic domove _posm;
				sleep 1;
				...
				if (unitReady _medic) then {
					...
					if (_medic distance _posm >0.2) then {
						_medic setVariable ["ROS_OPFOR_medicOrigPos",[],true];
						_medic setVariable ["ROS_OPFOR_medicOrigDir",0,true];
					};
				};
			};
		};
		...
		if (lifestate _incap != "incapacitated" && behaviour _incap != "COMBAT" && isNull _nearEnemy) then {
			if !((_incap getVariable ["ROS_OPFOR_OrigPos",[]]) isEqualTo []) then {
				_posi = (_incap getVariable ["ROS_OPFOR_OrigPos",[]]);
				_diri = (_incap getVariable ["ROS_OPFOR_OrigDir",0]);
				_incap domove _posi;
				sleep 1;
				...
				if (unitReady _incap) then {
					...
					if (_incap distance _posi >0.2) then {
						_incap setVariable ["ROS_OPFOR_OrigPos",[],true];
						_incap setVariable ["ROS_OPFOR_OrigDir",0,true];
					};
				};
			};
		};
...
if (isServer) then {
		...
		while {true} do {

			{
				...
				if (lifestate _x == "INCAPACITATED" && isTouchingGround _x && vehicle _x == _x) then {
					if ((_x getVariable ["ROS_OPFOR_IncapMarker",""]) == "") then {
						...
						_x setVariable ["ROS_OPFOR_IncapMarker",_mkrName,true];
					};
				};
				// Remove marker
				if (lifestate _x != "INCAPACITATED" && vehicle _x == _x && alive _x) then {
					...
					if !(_x getVariable ["ROS_OPFOR_IncapMarker",""] == "") then {
						deleteMarker (_x getVariable "ROS_OPFOR_IncapMarker");
						_x setVariable ["ROS_OPFOR_IncapMarker","",true];
					};
				};
				// Add dead marker
				if (!alive _x) then {
					if ((_x getVariable ["ROS_OPFOR_IncapMarker",""]) == "") then {
						...
						_mkr setMarkerSize [0.5,0.5];
						_x setVariable ["ROS_OPFOR_IncapMarker",_mkrName,true];
						if (ROS_OPFOR_statusmsg) then {
							["memberdied"] remoteexec ["playSound", allPlayers];
						};
					};
				};
				// AI heal themselves
				if (!isPlayer _x && !(_x in ROS_OPFOR_Processing) && alive _x && damage _x >=0.2) then {
					_x spawn ROS_OPFOR_Fnc_AI_healSelf;
				};
	...
	[] spawn {
		while {true} do {
			// Update ALLUNITS array
			[] call ROS_OPFOR_Fnc_unitArrayEHupdate; // returns ROS_OPFOR_AllUnitsArray / PubVar
	...
	[] spawn {
		while {true} do {
			...
			// Remove blood
			_bRemoveTime = 300;
			if (count ROS_OPFOR_bloodPools >0) then {
				sleep _bRemoveTime;
				deleteVehicle (ROS_OPFOR_bloodPools select 0);
			};
...
if (isServer) then {
	...
	ROS_OPFOR_incapsUnprocessed = [];
	ROS_OPFOR_medicsUnprocessed = [];

	// Store Orig Pos and Dir for units in own group
	{
		if (count units group _x ==1) then {
			if (_x getVariable ["ROS_OPFOR_OrigPos",[]] isEqualTo []) then {
				_pos = (getPosATL _x);
				_dir = (getdir _x);
				_x setVariable ["ROS_OPFOR_OrigPos", _pos, true];
				_x setVariable ["ROS_OPFOR_OrigDir", _dir, true];
			};
		};
	} foreach ROS_OPFOR_AllUnitsArray;

	// ROS_OPFOR_AI_revive Initialized
	// Zdo addition.
	...
	while {true} do {
		...
		call ROS_OPFOR_Fnc_UpdateIncapAndMedics;

		// Get array of unprocessed Incap units
		ROS_OPFOR_incapsUnprocessed = ROS_OPFOR_incapacitated select {!(_x in ROS_OPFOR_Processing)};

		if (count ROS_OPFOR_incapsUnprocessed >0) then {

			//***** SELECT INCAP UNIT *****//
			_incap = (ROS_OPFOR_incapsUnprocessed select 0);
			//[_incap,false] remoteExec ["allowDamage",_incap];
			[_incap,true] remoteExec ["setCaptive",_incap];
			ROS_OPFOR_Processing pushBackUnique _incap;
			publicVariable "ROS_OPFOR_Processing";
			...
			//***** SELECT MEDIC UNIT *****//
			ROS_OPFOR_medics_healthy = ROS_OPFOR_AllUnitsArray - ROS_OPFOR_incapacitated;
			ROS_OPFOR_medicsUnprocessed = (ROS_OPFOR_medics_healthy select {!(side _x == civilian) && !isPlayer _x && !(_x in ROS_OPFOR_Processing) && ((_x distance2D _incap) < ROS_OPFOR_medicDist)});

			if (alive _incap && count ROS_OPFOR_medicsUnprocessed >0) then {
				ROS_OPFOR_medics = [ROS_OPFOR_medicsUnprocessed, [], {_x distance _incap }, "ASCEND"] call BIS_fnc_sortBy;
				_medic = ROS_OPFOR_medics select 0;
				ROS_OPFOR_Processing pushBackUnique _medic;
				publicVariable "ROS_OPFOR_Processing";
				//[_medic,false] remoteExec ["allowDamage",_medic];
				[_medic,true] remoteExec ["setCaptive",_medic];
				sleep 0.2;
			...
			if (alive _incap && alive _medic && count units group _medic ==1) then {
				if (_medic getVariable ["ROS_OPFOR_medicOrigPos",[]] isEqualTo []) then {
					...
					_dir = (getdir _medic);
					_medic setVariable ["ROS_OPFOR_medicOrigPos", _pos, true];
					_medic setVariable ["ROS_OPFOR_medicOrigDir", _dir, true];
				};
			};

			// Player is down send text and sound message
			if (alive _incap && isPlayer _incap && alive _medic && ROS_OPFOR_statusmsg) then {
				_sound = "";
				_nearestUnits = (nearestObjects [_incap,["camanbase"],100]) select {side _x == ROS_OPFOR_RevSide};
			...
			if (alive _incap && alive _medic) then {
				...
				_incap setVariable ["incapTimeLimit", (time + ROS_OPFOR_maxIncapTime), true];
				...
				if (ROS_OPFOR_statusmsg && isPlayer _incap) then {
				...
				[_medic, _incap] spawn {
					...
					[_medic, _incap] spawn ROS_OPFOR_Fnc_Recycle_Fnc;

					// Start AI revive
					[_medic, _incap] call ROS_OPFOR_Fnc_AIReviveUnits;

					waitUntil {
						sleep 1;
						(!alive _incap or !alive _medic or !(lifestate _incap == "INCAPACITATED") or (!(_incap in ROS_OPFOR_Processing) or !(_medic in ROS_OPFOR_Processing)))
					};
				};

			}; // end if !(_medic in ROS_OPFOR_Processing) && !(_incap in ROS_OPFOR_Processing)

		}; // end count ROS_OPFOR_incapsUnprocessed >0 && count ROS_OPFOR_medics_healthy >0
